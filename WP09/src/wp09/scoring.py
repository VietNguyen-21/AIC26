...
class Siglip2Scorer:
    ...
    def __init__(self, model_name: str = "google/siglip2-base-patch16-224", *, batch_size: int = 1) -> None:
    ...
    def score(self, query_text: str, frames: Sequence[DecodedFrame]) -> tuple[float, ...]:
        ...
        try:
            for offset in range(0, len(frames), self._batch_size):
                images = [frame.image_rgb for frame in frames[offset: offset + self._batch_size]]
...
def _failure_reason(error: BaseException) -> str:
    return "scorer_oom" if "out of memory" in str(error).lower() else "scorer_unavailable"
